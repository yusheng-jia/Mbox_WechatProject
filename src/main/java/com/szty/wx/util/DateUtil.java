...
public class DateUtil {
     ...
     * @return 天
     */
    public static String getTimeAndMin(Date date) {
        Calendar calendar = Calendar.getInstance();
        int hour = 0;
        int min = 0 ;
        if (date != null) {
     ...
     * @param @param cacheTime 间隔时间
     *
     * @return Boolean 返回类型 true:未过期false:过期
     * @throws
     */
    public static Boolean isTimeExpiresIn(Date createTime, Long expires_in ,Long cacheTime) {
        if (createTime == null) {
            return false;
        }
        if(expires_in == null){
        ...
        Long crete = createTime.getTime();
        Long expires = Long.valueOf(expires_in);
        if (now - crete <= expires - cacheTime) {
            return true;
        }
        return false;
    ...
    public static boolean isValidDate(String str,DateStyle dataStyle) {
        // String str = "2007-01-02";
        DateFormat formatter = new SimpleDateFormat(dataStyle.getValue());
        try {
    ...
    public static void main(String[] args) {
        System.out.println(isValidDate("2010-01-02 ",DateStyle.YYYY_MM_DD));
    }
}
